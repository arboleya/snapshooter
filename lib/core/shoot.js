  ...
  */
  ...
  module.exports = Shoot = (function() {
    ...
    function Shoot(the, cli) {
      ...
      first_url = this.cli.argv.input.replace(/\/index\.\w+$/m, '');
    ...
    Shoot.prototype.after_crawl = function(source) {
      ...
      if (source != null) {
        while ((match = reg.exec(source)) != null) {
          relative = match[1];
          absolute = this.domain + ("/" + relative).replace(/^\/\//, '/');
    ...
    Shoot.prototype.save_page = function(url, source) {
      var $, hook, output_file, output_folder, relative_url;
      if (this.cli.argv.hook) {
        $ = require('jquery').create();
        $ = $(source);
        hook = require(process.cwd() + '/' + this.cli.argv.hook);
    ...
    Shoot.prototype.url_is_passing = function(relative, absolute) {
      var flag, flags, not_anchor, not_backwards, not_crawled, not_exclude, not_image, not_mail_to, not_pdf, not_slash, not_zip, passed, _i, _len;
      not_slash = absolute !== '/';
      ...
      not_mail_to = !(/^mailto:/m.test(relative));
      ...
      flags = [not_slash, not_crawled, not_anchor, not_image, not_zip, not_pdf, not_exclude, not_backwards, not_mail_to];
      passed = true;
